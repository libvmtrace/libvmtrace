...
void AddWriteFiles(const Process& p, vmi_pid_t parent, uint64_t fd, string path);
	...
	public:
		bool callback(const Event* ev, void* data)
		{
			...
			if(sev)
			{
				...
				// {
				// 	cout << "====" << endl;
				// 	vector<OpenFile> aff = _linux->GetOpenFiles(p);
				// 	for (vector<OpenFile>::iterator ita = aff.begin() ; ita != aff.end(); ++ita)
				// 	{
				// 		cout << dec << "\t" << (*ita).fd << " - " << (*ita).path << endl;
				// 	}
				// }
					...
					if(it2 != sshchildren.end())
					{
						...
						for (vector<OpenFile>::iterator ita = af.begin() ; ita != af.end(); ++ita)
						{
							if((*ita).write)
							{
								...
								{
									path = (*ita).path;
									find_and_replace(path, "/", "_");

									ofstream binFile2(_log_dir+to_string((*it2).pid)+"_"+to_string(p.GetPid())+"_"+p.GetName()+"_"+to_string(fd)+"---"+path, ios::out | ios::binary | ios_base::app);
									if (binFile2.is_open())
									{
									...
									// AddWriteFiles(p, parent, fd, path);
									break;
								}
							}
						}
						...
						// vector<writefile>::iterator it3 = find_if(writefiles.begin(), writefiles.end(), writefind(p.GetPid(), parent, fd, path));
		...
		bool callback(const Event* ev, void* data)
		{
			...
			if(sev)
			{
				...
				if(it2 != sshchildren.end())
				{
					...
					vector<writefile>::iterator it3 = find_if(writefiles.begin(), writefiles.end(), writefind(p.GetPid(),parent,s->GetParameter(0),path));
					if(it3 != writefiles.end())
					{
						(*it3).fseek = true;
	...
	public:
		bool callback(const Event* ev, void* data)
		{
			...
			if(sev)
			{
				...
				it = find_if(sshchildren.begin(), sshchildren.end(), boost::bind(&sshchild::pid, _1) == parent);
				if (it != sshchildren.end())
				{
					(*it).sshAddr = p1;
	...
	public:
		...
		void callback(Packet* p)
		{
			...
			try
			{
				...
				if (it != sshchildren.end())
				{
					get(std::to_string((*it).pid)+"net")->info("New Connection - From : [{0}:{1:d}] To : [{2}:{3:d}] Proc Name : [{4}] Pid : [{5:d}]",(string)p->get_ipv4_src(),p->get_sport(),(string)p->get_ipv4_dst(), p->get_dport(), process.GetName(), process.GetPid());
				}
			}
			catch(...)
			{
...
void AddWriteFiles(const Process& p, vmi_pid_t parent, uint64_t fd, string path)
{
	vector<writefile>::iterator it33 = find_if(writefiles.begin(), writefiles.end(), writefind(p.GetPid(), parent, fd, path));
	if(it33 == writefiles.end())
	{
		vector<OpenFile> af = _linux->GetOpenFiles(p);
		for (vector<OpenFile>::iterator ita = af.begin() ; ita != af.end(); ++ita)
		{
			if((*ita).write)
			{
				...
				if((*ita).path != "/dev/null" && (*ita).fd == fd)
				{
					writefile wf;
					wf.pid = p.GetPid();
					wf.parent = parent;
					...
					// cout << "------ " << p.GetName() << " ------" << endl;
					// cout << "\tpath : " << wf.path << endl;
					// cout << "\tfd1 : " << dec << fd << endl;
					// cout << "\tfd2 : " << dec << (*ita).fd << endl;
